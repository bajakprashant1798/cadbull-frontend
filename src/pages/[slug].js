...
const CadLandscaping = ({ initialProjects, initialTotalPages, initialSlug }) => {
  ...
  useEffect(() => {
    if (!slug) return; // skip if slug is not yet defined
    loadProjects();
  }, [subcatfilter, slug]);
...
export async function getStaticPaths() {
  // Fetch all categories and subcategories to pre-build their pages
  const catRes = await getallCategories("");
  const categories = catRes?.data?.categories || [];

  // If you also have subcategories as separate pages, fetch those slugs too
  // (Assuming getallsubCategories() returns slugs for all subcategories)
  ...
  // If subcategories are top-level, add those too
  // paths.push(...subcategories.map((sub) => ({ params: { slug: sub.slug } })));
...
export async function getStaticProps({ params }) {
  ...
  // Fetch category or subcategory info by slug
  const data = await getSubCategories({ slug, currentPage: 1, pageSize: 9 });
